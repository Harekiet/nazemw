...
void beep_code(char first, char second, char third, char pause)
{
    ...
    if (icnt < 3 && Duration != 0)
        beep(Duration);
    if (icnt >= 3 && (buzzerLastToggleTime < timeMilli() - Duration)) {
        icnt = 0;
        toggleBeep = 0;
    }
...
static void beep(uint16_t pulse)
{
    if (!buzzerIsOn && (timeMilli() >= (buzzerLastToggleTime + 50))) {         // Buzzer is off and long pause time is up -> turn it on
        buzzerIsOn = 1;
        boardBeepOn();
        buzzerLastToggleTime = timeMilli();      // save the time the buzer turned on
    } else if (buzzerIsOn && (timeMilli() >= buzzerLastToggleTime + pulse)) {         // Buzzer is on and time is up -> turn it off
        buzzerIsOn = 0;
        boardBeepOff();
        buzzerLastToggleTime = timeMilli();
        if (toggleBeep >0)
            toggleBeep--;
        beepDone = 1;
    }
