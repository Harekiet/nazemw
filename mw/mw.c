...
//    static uint8_t ind = 0;
    ...
    //When the gps has 5 or more satellites
...
uint16_t pwmReadRawRC(uint8_t chan)
{
    ...
    data = boardPWMInputSingle( mcfg.rcmap[chan] );
...
void computeRC(void)
{
    static uint16_t rcData4Values[4][8];
    static uint8_t rc4ValuesIndex = 0;
    uint8_t chan, a;

    //Forward the index where to write the new value
    rc4ValuesIndex = (rc4ValuesIndex + 1 ) % 4;
    //Read all 8 channels at once
    boardPWMInputMultiple( 0, 8, rcData4Values[rc4ValuesIndex] );
    //Calculate the rc values based on mean of 4 last readings
    for (chan = 0; chan < 8; chan++) {
    	uint16_t mean = 0;
        for (a = 0; a < 4; a++)
            mean += rcData4Values[a][chan];

        mean = (mean + 2) / 4;
        if (mean < rcData[chan] - 3)
            rcData[chan] = mean + 2;
        if (mean > rcData[chan] + 3)
            rcData[chan] = mean - 2;
    }
