...
static void writeSerial( uint8_t b ) {

}

static void sendDataHead(uint8_t id)
{
    writeSerial(PROTOCOL_HEADER);
    writeSerial(id);
}

static void sendTelemetryTail(void)
{
    writeSerial(PROTOCOL_TAIL);
}
...
static void sendAccel(void)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        serialize16( (accSmooth[i] * 1000U ) / ACC_1G );
    }
}
...
{
    uint32_t seconds = timeMilli() / 1000;
...
{
    if (timeMilli() - lastCycleTime >= CYCLETIME) {
        lastCycleTime = timeMilli();
        cycleNum++;
